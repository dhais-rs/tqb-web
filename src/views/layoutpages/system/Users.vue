...
const handelSwitch = async (val, row) => {
    ...
    row.load = 1;
    const { code } = await VE_API.system.userState(
        {
            id: row.id,
            state: val,
        },
...
const handleDel = (id) => {
    proxy
        ...
        .then(async () => {
            const { code } = await VE_API.system.delUser({ id });
            if (code == "1") {
                getDataList();
            }
        })
...
const getDataList = async () => {
    loading.value = true;
    const data = await VE_API.system.userList(params);
    if (data.code == "1") {
        const { code, info, total, row } = data;
        params.limit = params.currentNum;
